...
function UserModel (userCollection) {
    ...
    this.checkLoginToken = function checkLoginToken(loginToken, callback) {
        
        userCollection.findOne({loginToken: loginToken}, function(err, access) {

            if(err) {
                callback(err);
            } else if(access) {
                callback(null, access._id);
            } else {
                callback(null, null);
            }
        });
